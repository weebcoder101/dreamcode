...
# Dream Gate Workflow (CRITICAL — Agent Must Follow)
...
**CRITICAL ordering rule — plan text must be the LAST text before the edit.** The gate only reads text accumulated after the most recent tool result; every tool result flushes that buffer. If you write the plan and THEN call relations/read/grep, the plan is invisible by the time the edit runs → hard block. Always order: correlation calls first, then plan text, then the edit — never plan-before-correlation.
...
# Effect 4.0.0 ScopedCache Deadlock Pattern (Critical)

**Problem**: `ScopedCache.get` runs lookups on the **caller's fiber** (not forked). When `InstanceState.get` (which wraps `ScopedCache`) is called from inside another `InstanceState.get` on the same fiber, the nested lookup deadlocks — the outer lookup's scope is still open, and `Scope.provide` for the inner lookup waits on the outer scope's closure.

**Symptoms**: Test hangs at 5s timeout with no error; `DBG` logs show lookup START but never completes; key computation works but lookup never runs.

**Root cause**: Cold caches (empty `ScopedCache` map) → miss path creates new entry → `Scope.provide(restore(lookup), scope)` runs on same fiber as outer lookup → deadlock.

**Fixes**:
1. **Pre-warm at layer scope**: Call `config.get()`, `plugin.list()`, `agent.list()` in the layer's construction Effect (before `InstanceState.make`) so caches are hot when state build runs.
2. **Hoist service reads out of lookups**: Move `config.get()` to plugin layer scope (before `InstanceState.make`) so the lookup closes over it — avoids nested cache access entirely.
3. **Use `Layer.provideMerge` for shared services**: `ToolRegistry.defaultLayer` internally `provide`s 18 services (Config, Plugin, Agent, etc.) — these shadow outer layer provides. Tests must use `ToolRegistry.layer` + explicit provides to share instances.

**Detection**: Add `labelCache(cache, "name")` via `WeakMap` + log in `InstanceState.get` — different cache instances = different labels (`undefined` = separate cache).

**Effect version**: 4.0.0-beta.74 confirmed; fixed in later betas? Unknown — defensive patterns required.

---

# Parent Agent Model Isolation (Critical)

**Problem**: When subagents complete and their results are injected back into the parent session, the parent agent's user-selected model can be silently overwritten to the subagent's model. The user picks Ox Alpha; the parent switches to DeepSeek.

**Symptoms**: Parent model in TUI changes after subagent completion; `SessionTable.model` in DB shows wrong model; subsequent LLM calls use wrong model.

**Root cause**: `queueSynthetic` and `inject` in `task.ts` used `parentMsgModel ?? model` as the model for injected results. When `parentMsgModel` was undefined (compacted messages, DB races, schema edge cases), the fallback `model` was the **subagent's** model. This leaked via `ModelSwitched` events in `prompt-user-message.ts`.

**Fix**: Introduced `guaranteedParentModel` — a 3-tier fallback:
1. `parentMsgModel` (from parent assistant message)
2. `SessionTable.model` (authoritative DB source)
3. First user message's model

All three injection paths use `guaranteedParentModel`. The subagent's model can never leak.

**Key rule**: `parentMsgModel ?? model` is ALWAYS wrong in injection paths. `model` in task.ts scope is the **subagent's** model — never use it for parent session operations.

**File**: `packages/opencode/src/tool/task.ts`
**Docs**: `docs/PARENT-MODEL-ISOLATION-FIX.md`
