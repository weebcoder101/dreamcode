...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run dreamcode with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running dreamcode server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
      ...
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to OPENCODE_SERVER_USERNAME or 'dreamcode')",
      })
      .option("dir", {
        type: "string",
      ...
      async function execute(sdk: OpencodeClient) {
        ...
        try {
          await runInteractiveMode({
            ...
            thinking,
            backgroundSubagents: flags.backgroundSubagents,
            demo: args.demo,
          })
        } catch (error) {
          dieInteractive(error)
