...
const TUI_SCHEMA_URL = "https://dreamcode.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from dreamcode.json files
...
async function opencodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "dreamcode"),
    ...(await Filesystem.findUp(["dreamcode.json", "dreamcode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "dreamcode"))
  }
