...
export class Service extends Context.Service<Service, Interface>()("@dreamcode/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["dreamcode.jsonc", "dreamcode.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://dreamcode.ai/config.json"
        const updated = text.replace(/^\s*\{/, '{\n  "$schema": "https://dreamcode.ai/config.json",')
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.OPENCODE_CONFIG && !Flag.OPENCODE_CONFIG_DIR && !Flag.OPENCODE_CONFIG_CONTENT) {
        ...
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://dreamcode.ai/config.json" }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "dreamcode.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "dreamcode.jsonc"), env))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://dreamcode.ai/config.json"
      ...
      function* (ctx: InstanceContext) {
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/dreamcode`
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://dreamcode.ai/config.json"
        ...
        if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("dreamcode", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        for (const dir of directories) {
          ...
          // Auto-discovered plugins under `.dreamcode/plugin(s)` are already local files, so ConfigPlugin.load
