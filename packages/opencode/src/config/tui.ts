...
export class Service extends Context.Service<Service, Interface>()("@dreamcode/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  // 4. `.dreamcode` / `.opencode` directories (and OPENCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".dreamcode") || dir.endsWith(".opencode") || dir === Flag.OPENCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".dreamcode") && !dir.endsWith(".opencode") && dir !== Flag.OPENCODE_CONFIG_DIR) continue
