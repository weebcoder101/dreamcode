      ...
      execute: (args: Schema.Schema.Type<typeof Parameters>, ctx: Tool.Context) =>
        Effect.gen(function* () {
          ...
          if (!grammar) {
            ...
            throw new Error(
              `No tree-sitter grammar for ${ext} files (${path.basename(filePath)}). ` +
              `Use the \"edit\" tool instead — ast-edit only supports: ` +
              `TypeScript (.ts/.tsx), JavaScript (.js/.jsx), Python (.py), Rust (.rs), Go (.go), ` +
              `Java (.java), Ruby (.rb), PHP (.php), C# (.cs), C/C++ (.c/.cc/.cpp/.h/.hpp), ` +
              `Scala (.scala), Haskell (.hs), Elixir (.ex/.exs), Dart (.dart), Julia (.jl), ` +
              `OCaml (.ml/.mli), JSON, HTML, CSS, Svelte (.svelte), Bash (.sh/.bash), PowerShell (.ps1).`,
            )
          }
