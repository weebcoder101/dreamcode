...
export class Service extends Context.Service<Service, Interface>()("@dreamcode/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `dreamcode auth login` in the terminal",
      name: "Login with dreamcode",
      id: AuthMethodID,
    }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "DreamCode service failure", service })
}
