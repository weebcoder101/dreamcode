...
export class Service extends Context.Service<Service, Interface>()("@dreamcode/AccountRepo") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const current = Effect.fnUntraced(function* () {
      const state = yield* Effect.orDie(db.select().from(AccountStateTable).where(eq(AccountStateTable.id, ACCOUNT_STATE_ID)).get())
      if (!state?.active_account_id) return
      const account = yield* Effect.orDie(db.select().from(AccountTable).where(eq(AccountTable.id, state.active_account_id)).get())
      if (!account) return
