...
export async function spawnWslSidecar(
  ...
  const script = [
    ...
    // SECURITY: this is a per-launch, in-memory secret. The password
    // is generated by Electron and passed through WSLENV to the opencode
    // server inside the WSL distro. It must NOT be logged, persisted to
    // disk on the Windows side, or echoed back through stdin/stdout.
    // The `REDACTED_xxxxxxxx` value above is a per-launch token; the
    // real value is interpolated at runtime from Electron's main process.
