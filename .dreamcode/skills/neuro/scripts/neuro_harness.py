...
def main():
    ...
    SYSTEM_PROMPTS = {
        "full_audit": "You are an expert software architect reviewing a codebase. Analyze for architecture issues, security vulnerabilities, performance bottlenecks, code quality, and testing gaps.",
        "security": "You are a security expert reviewing code for vulnerabilities. Focus on OWASP Top 10, auth flaws, input validation, secrets exposure, and dependency vulnerabilities.",
        "bug_hunt": "You are a debugging expert analyzing code for bugs. Focus on logic errors, race conditions, memory leaks, error handling, and type safety.",
        "test_gap": "You are a testing expert analyzing code coverage gaps. Focus on missing unit tests, integration tests, edge cases, and error paths.",
    }
    
    system_prompt = SYSTEM_PROMPTS.get(args.scan_type, SYSTEM_PROMPTS["full_audit"])
    file_contents = []
    for f in files:
